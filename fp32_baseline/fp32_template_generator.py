...
generated_cores/ (repository root) so that FPGA/ASIC results can be compared directly:
  ...
  * TOTAL_LATENCY = 13 (11 cycles operand alignment + 2 cycles for the
    ...
    STALL_CYCLES = TOTAL_LATENCY + 1 inter-stage stall. This is 2 cycles more
    than the mixed-precision/FP16 baselines' TOTAL_LATENCY = 11, because the
    FP32 butterfly's multiply->add->add chain does not meet a 10ns clock
    fully combinationally at 45nm generic-cell synthesis; cycle counts per
    transform are correspondingly 2*num_stages higher, not identical
...
# The shared butterfly (fp32_butterfly.v) is internally pipelined 2 cycles
# deep (multiply -> combine-add -> final-add, one arithmetic primitive per
# stage) to meet a 10ns clock at 45nm generic-cell synthesis -- see the
# header comment in fp32_butterfly.v for the timing rationale. The operand
# (A/B) and twiddle pipelines feed the butterfly's INPUTS and are unchanged;
# only the write-back address/enable pipeline, which waits for the
# butterfly's OUTPUT, needs to grow by those same 2 cycles.
BUTTERFLY_LATENCY = 2
