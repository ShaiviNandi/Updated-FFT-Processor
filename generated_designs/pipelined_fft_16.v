...
module pipelined_fft_16_core #(
    ...
    input  wire                  ext_wr_en, input  wire [ADDR_WIDTH-1:0] ext_wr_addr, input  wire [23:0]           ext_wr_data,
    input  wire                  ext_reading, input  wire [ADDR_WIDTH-1:0] ext_rd_addr, output wire [15:0]           ext_rd_data,
    ...
    reg cur_mult_prec; reg cur_rd_prec; reg cur_wr_prec;
    ...
    dit_fft_agu_streaming #( .MAX_N(MAX_N), .ADDR_WIDTH(ADDR_WIDTH) ) agu (
        .clk(clk), .reset(rst), .start(start_agu_reg), .N(11'd16), .stream_en(streaming_enable),
        .idx_a(idx_a), .idx_b(idx_b), .k(k), .done_stage(done_stage), .done_fft(done_fft), .curr_stage(curr_stage)
    );
    ...
    always @(*) begin
        if (ext_reading) begin
            ...
            case (curr_stage[3:0])
            4'd0: begin cur_mult_prec = STAGE0_MULT_PREC; cur_rd_prec = 1'b1; end
            4'd1: begin cur_mult_prec = STAGE1_MULT_PREC; cur_rd_prec = STAGE0_OUT_PREC; end
            4'd2: begin cur_mult_prec = STAGE2_MULT_PREC; cur_rd_prec = STAGE1_OUT_PREC; end
            4'd3: begin cur_mult_prec = STAGE3_MULT_PREC; cur_rd_prec = STAGE2_OUT_PREC; end
                default: begin cur_mult_prec = 1'b0; cur_rd_prec = 1'b1; end
            endcase
        end
    end
    ...
    always @(*) begin
        case (curr_stage_delayed[3:0])
            4'd0: begin cur_wr_prec = STAGE0_OUT_PREC; end
            4'd1: begin cur_wr_prec = STAGE1_OUT_PREC; end
            4'd2: begin cur_wr_prec = STAGE2_OUT_PREC; end
            4'd3: begin cur_wr_prec = STAGE3_OUT_PREC; end
            default: cur_wr_prec = 1'b0;
        endcase
    end

    wire [15:0] twiddle;
    cordic_twiddle_generator #( .LATENCY(10), .MAX_N(MAX_N), .ADDR_WIDTH(ADDR_WIDTH) ) twiddle_gen (
        .clk(clk), .rst(rst), .k(k), .n(11'd16), .valid_in(streaming_enable), .PRECISION(cur_mult_prec), .twiddle_out(twiddle)
    );

    // Hazard-Free Delayed Pipeline Alignment Matrix
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            for (i = 0; i < TOTAL_LATENCY; i = i + 1) begin
                wr_addr_a_pipe[i] <= 0; wr_addr_b_pipe[i] <= 0; bank_sel_pipe[i] <= 0; stage_mask_pipe[i] <= 0; curr_stage_pipe[i] <= 0;
            end
        end else begin
    ...
    mixed_dual_bank_memory_concurrent mem (
        .clk(clk), .rst(rst),
        .bank_pingpong(active_rd_bank), 
        .stage_mask(ext_reading ? 11'h001 : current_rd_mask), 
        .rd_addr_a(mem_rd_addr_a), .rd_addr_b(mem_rd_addr_b), .rd_precision(cur_rd_prec), .rd_data_a(rd_data_a_16), .rd_data_b(rd_data_b_16),
        .wr_en(ext_wr_en ? 1'b1 : mem_wr_en), .wr_addr_a(ext_wr_en ? ext_wr_addr : mem_wr_addr_a), .wr_addr_b(ext_wr_en ? ext_wr_addr : mem_wr_addr_b),
        .wr_data_a(ext_wr_en ? ext_wr_data : X_wr_24), .wr_data_b(ext_wr_en ? ext_wr_data : Y_wr_24),
        // Pipeline hooks seamlessly override write assignments without altering basic structure
        .bank_pingpong_wr(ext_wr_en ? 1'b0 : mem_wr_bank),
        .stage_mask_wr(ext_wr_en ? 11'h001 : mem_wr_mask)
    );
    ...
    reg [23:0] A_24_pipe [0:7]; reg [23:0] B_24_pipe [0:7];
    always @(posedge clk) begin
        A_24_pipe[0] <= mem_rd_a_24; B_24_pipe[0] <= mem_rd_b_24;
        for (integer j = 1; j < 8; j = j + 1) begin
            A_24_pipe[j] <= A_24_pipe[j-1]; B_24_pipe[j] <= B_24_pipe[j-1];
        end
    end
    wire [23:0] A_24_aligned = A_24_pipe[7];
    wire [23:0] B_24_aligned = B_24_pipe[7];
    // Per-stage butterfly wrappers matrix
    wire [15:0] X_st0, Y_st0; wire fp8_out_st0;
    wire [15:0] X_st1, Y_st1; wire fp8_out_st1;
    wire [15:0] X_st2, Y_st2; wire fp8_out_st2;
    wire [15:0] X_st3, Y_st3; wire fp8_out_st3;
    butterfly_wrapper #( .MULT_PRECISION(1), .ADD_PRECISION(1) ) bf_st0 (
        .A(A_24_aligned), .B(B_24_aligned), .W(twiddle), .X(X_st0), .Y(Y_st0), .output_is_fp8(fp8_out_st0)
    );
    butterfly_wrapper #( .MULT_PRECISION(0), .ADD_PRECISION(0) ) bf_st1 (
        .A(A_24_aligned), .B(B_24_aligned), .W(twiddle), .X(X_st1), .Y(Y_st1), .output_is_fp8(fp8_out_st1)
    );
    butterfly_wrapper #( .MULT_PRECISION(1), .ADD_PRECISION(0) ) bf_st2 (
        .A(A_24_aligned), .B(B_24_aligned), .W(twiddle), .X(X_st2), .Y(Y_st2), .output_is_fp8(fp8_out_st2)
    );
    butterfly_wrapper #( .MULT_PRECISION(1), .ADD_PRECISION(1) ) bf_st3 (
        .A(A_24_aligned), .B(B_24_aligned), .W(twiddle), .X(X_st3), .Y(Y_st3), .output_is_fp8(fp8_out_st3)
    );
    reg [15:0] X_bf, Y_bf; reg bf_is_fp8;
    always @(*) begin
        case (curr_stage_delayed[3:0])
    ...
    reg [1:0] state; reg [5:0] flush_counter;
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            state <= 0; start_agu_reg <= 0; fft_bank_sel <= 0; done <= 0; flush_counter <= 0;
        end else begin
            case (state)
                0: begin
                    done <= 1'b0;
                    if (start) begin fft_bank_sel <= 1'b0; start_agu_reg <= 1'b1; state <= 1; end
                end
                1: begin
                    start_agu_reg <= 1'b0;
                    if (done_stage) fft_bank_sel <= ~fft_bank_sel;
                    if (done_fft) begin state <= 2; flush_counter <= TOTAL_LATENCY; end
                end
                2: begin
                    if (flush_counter == 0) begin fft_bank_sel <= 1'b0; done <= 1'b1; state <= 3; end
                    else flush_counter <= flush_counter - 1;
                end
                3: if (!start) begin done <= 1'b0; state <= 0; end
            endcase
        end
    end
endmodule
